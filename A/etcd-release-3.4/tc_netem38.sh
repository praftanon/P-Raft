#!/bin/bash
# tc_netem38.sh
# 在 192.168.0.38 上配置不同目的 IP 的网络延迟
...
# 2. 为目标 156 创建 15ms 延迟通道 (band 3)
sudo tc qdisc add dev $DEV parent 1:3 handle 30: netem delay 15ms

# 3. 为目标 93 创建 20ms 延迟通道 (band 1)
sudo tc qdisc add dev $DEV parent 1:1 handle 40: netem delay 20ms

# 4. 创建过滤器，将发往 192.168.0.82 的流量导向 band 3
sudo tc filter add dev $DEV protocol ip parent 1:0 prio 1 u32 \
    match ip dst 192.168.0.82/32 flowid 1:3

# 5. 创建过滤器，将发往 192.168.0.223 的流量导向 band 1
