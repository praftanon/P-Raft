...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
         ...
         type=int,
         default=None,
         help="每个大窗口内的10s子请求重复次数；默认=duration/10（例如 duration=20 ⇒ 2）",
    )
...
def append_takes_from_log(log_path: str, times_path: str = "/etcd/etcd-release-3.4/timesC.csv") -> None:
    """
...
def append_total_stats_from_log(
    log_path: str,
    times_path: str = "/etcd/etcd-release-3.4/timesC.csv"
) -> None:
    """
...
def main() -> None:
    ...
    if win % 10 != 0:
        sys.exit(f"ERROR: --duration({win}) 必须是 10 的倍数，因为我们以 10s 为子窗口")
    sub_dur = 10
    sub_repeat = args.sub_repeat if args.sub_repeat is not None else (win // 10)
    if sub_repeat <= 0:
        sys.exit("ERROR: --sub-repeat 必须为正整数")
    n = len(endpoints)
    ...
    try:
        while True:
             ...
             # —— 用本“大窗口”的矩阵行来生成每个 10s 子窗口的 opcount（A×sub_repeat）
             row = slot % args.load_rows
             p = float(M[row, domain_col])
             opcount_each = max(1, int(round(args.load_base * p)))
 
             for r in range(sub_repeat):
                 sub_start = slot_start + r * sub_dur
                 sub_end   = sub_start + sub_dur
 
                 # 等待子窗口起点
                 now = time.time()
                 if now < sub_start:
                     time.sleep(max(0.0, sub_start - now))
 
                 ts = datetime.now().strftime('%Y%m%d_%H%M%S')
                 log(f"=== SLOT {subslot} @ {my_ep} for {sub_dur}s, opcount={opcount_each}, p={p:.4f} ===")
                 log_file = log_dir / f"slot{subslot}_MATRIX_{my_ep.replace(':','_')}_{ts}.log"
 
                 proc = start_ycsb(
                     goycsb, args.db, workload, my_ep, args.threads, extra_props, log_file, opcount_each
                 )
             ...
             log(f"END SLOT {slot} (completed {sub_repeat}×10s sub-windows)")
