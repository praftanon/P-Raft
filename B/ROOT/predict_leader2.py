...
class OptimalLeaderCalculator:
    ...
    def move_leader_with_timing(self, cmd, csv_path="/etcd/etcd-release-3.4/move_leader_timeB.csv"):
        # 记录起始时间
        t_start = time.time()
        try:
            subprocess.run(cmd, check=True)
...
PREDICT_LOG_CSV = "/etcd/etcd-release-3.4/predicted_leaderB.csv"
METRICS_CSV     = "/etcd/etcd-release-3.4/leader_move_metricsB.csv"

def _compute_align_targets(mode: int):
    if mode == 1:
        return [8, 18, 28, 38, 48, 58]
    if mode == 2:
        return [18, 38, 58]
    if mode == 3:
        return [28, 58]
...
ALIGN_TARGETS = _compute_align_targets(ALIGN_MODE)
...
def main():
    ...
    print(f"[CONF] USE_LAST_ROWS={USE_LAST_ROWS}, FIXED_STEP_SEC={FIXED_STEP_SEC}, MAX_ITERS={MAX_ITERS}")
    print("[CONF] ALIGN = 每分钟 :08/:18/:28/:38/:48/:58")
    ...
    try: 
        while True:
            _sleep_until_targets(ALIGN_TARGETS)
            it += 1
            print(f"\n===== 回合 #{it} =====")
            ...
            if not will_move:
                ...
                if MAX_ITERS > 0 and it >= MAX_ITERS:
                    ...
                    break
                print(f"[KEEP] predicted {self_ip}s 后再次评估...")
...
def _sleep_until_targets(target_seconds):
    """
    阻塞直到系统时间的“秒”命中 target_seconds（列表），例如：
    [8,18,28,38,48,58] 或 [18,38,58] 或 [28,58]
    """
    TARGETS = sorted(set(int(x) % 60 for x in target_seconds))
    SLOP = 0.05  # 抖动容忍（50ms）
    ...
    while True:
        ...
        # 找到“下一个目标秒”
        next_candidates = [t for t in TARGETS if t > sec or (t == sec and frac < SLOP)]
        next_sec = next_candidates[0] if next_candidates else (TARGETS[0] + 60)

        sleep_sec = next_sec - (sec + frac)
        if sleep_sec > 0:
            time.sleep(sleep_sec)

        # 命中确认（再取整校验一次，避免飘过）
        hit = int(time.time()) % 60
        if hit in TARGETS:
            return
